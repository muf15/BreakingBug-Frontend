...
const Products = ({}) => {
  ...
  const navigate = useNavigate(); // Added definition for navigate

  const itemsPerPage = 9;
  const { currentRole, responseSearch } = useSelector(state => state); // Fixed useSelector to get state
  ...
  return (
    <>
      <ProductGrid container spacing={3}>
        {currentItems.map((data, index) => (
          <Grid item xs={12} sm={6} md={4}
            key={index}
            onClick={() => navigate("/product/view/" + data._id)} // Fixed navigate usage
            sx={{ cursor: "pointer" }}
          >
            <ProductContainer>
      ...
      <Container sx={{ mt: 10, mb: 10, display: "flex", justifyContent: 'center', alignItems: "center" }}>
        <Pagination
          count={Math.ceil(responseSearch.length / itemsPerPage)} // Fixed usage of responseSearch instead of productData
          page={currentPage}
          color="secondary"
